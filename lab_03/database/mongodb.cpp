...
void Database::JSONToDocument(Poco::JSON::Object::Ptr json, Poco::MongoDB::Document& doc) {
    for (auto& service : *json) {
        if (service.second.isInteger()) {
            doc.add(service.first, service.second.extract<int>());
        } else if (service.second.isNumeric()) {
            doc.add(service.first, service.second.extract<double>());
        } else if (service.second.isArray()) {
            auto& arr = doc.addNewArray(service.first);
            for (auto& v : *service.second.extract<Poco::JSON::Array::Ptr>()) {
                arr.add(v.toString());
            }
        } else if (service.second.isString()) {
            doc.add(service.first, service.second.extract<std::string>());
        } else {
            try {
                Poco::JSON::Object::Ptr child = service.second.extract<Poco::JSON::Object::Ptr>();
                Poco::MongoDB::Document& child_doc = doc.addNewDocument(service.first);
                JSONToDocument(child, child_doc);
            } catch (...) {
                doc.add(service.first, service.second.toString());
            }
        }
    }
}
